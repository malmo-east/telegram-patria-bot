...
const db = require('../film_database.json')

helper.logStart()

mongoose.Promise = global.Promise
mongoose.connect(config.DB_PATRIA, {
  useMongoClient: true
})
...
// db.films.forEach(f => new Film(f).save().catch(e => console.log(e)))
// db.cinemas.forEach(c => new Cinema(c).save().catch(e => console.log(e)))
...
bot.on('message', msg => {
  ...
  switch (msg.text) {
    case kb.home.favourite:
      ...
      break
    case kb.home.films:
      bot.sendMessage(chatId, `Выберите кинотеатр:`, {
        reply_markup: {keyboard: keyboard.films}
      })
      break
    case kb.cinema_title.loteanu:
      bot.sendMessage(chatId, `Выберите рассписание`, {
        reply_markup: {keyboard: keyboard.schedule}
      })
      break
    case kb.cinema_title.multiplex:
      bot.sendMessage(chatId, `Выберите рассписание`, {
        reply_markup: {keyboard: keyboard.schedule}
      })
      break
    case kb.cinema_title.riscani:
      bot.sendMessage(chatId, `Выберите рассписание`, {
        reply_markup: {keyboard: keyboard.schedule}
      })
      break
    case kb.cinema_title.balti:
      bot.sendMessage(chatId, `Выберите рассписание`, {
        reply_markup: {keyboard: keyboard.schedule}
      })
      break
    case kb.date.today:
      sendFilmsByQuery(chatId, {date: 'today'})
      break
    case kb.date.tomorrow:
      sendFilmsByQuery(chatId, {date: 'tomorrow'})
      break
    case kb.date.after_tomorrow:
      sendFilmsByQuery(chatId, {date: 'after_tomorrow'})
      break
...
bot.on('callback_query', query => {
  ...
  if (type === ACTION_TYPE.SHOW_CINEMAS_MAP) {
    ...
    sendCinemasByQuery(userId, {uuid: {'$in': data.filmUuid}})
...
function sendFilmsByQuery(chatId, query) {
  Film.find(query).then(films => {
    const html = films.map((f, i) => {
      return `<b>${i + 1}</b> ${f.name} - ссылка(/f${f.uuid})`
    }).join('\n')
